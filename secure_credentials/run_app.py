...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("run_app_debug.log"), logging.StreamHandler()],
)
            ...
            for i in range(10):  # Try multiple times
                ...
                try:
                    if hasattr(app, "root") and app.root.winfo_exists():
                        ...
                        logger.debug(
                            f"Window state: visible={app.root.winfo_ismapped()}"
                        )
                except Exception as e:
...
def test_window():
    ...
    try:
        ...
        message = ttk.Label(
            root,
            text="If you can see this window, the issue is likely\nspecific to the credential manager GUI.",
            wraplength=500,
        )
        ...
        ttk.Button(
            ...
            command=force_visibility,
            style="primary.TButton",
        ).pack(pady=10)

        # Add a close button
        ttk.Button(
            root, text="Close", command=root.destroy, style="danger.TButton"
        ).pack(pady=10)
