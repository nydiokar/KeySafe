...
def dashboard():
    ...
    try:
        ...
        grouped_creds = {"passwords": [], "api_keys": [], "other": []}
...
def edit_credential(name):
    ...
    if request.method == "GET":
        ...
        try:
            if kp.edit_credential(
                name, new_name, new_value, new_type, current_password
            ):
                flash("Credential updated successfully!", "success")
                return redirect(url_for("dashboard"))
            else:
                flash(
                    "Failed to update credential. Check your current password.", "error"
                )
...
def api_view_credential(name):
    ...
    try:
        ...
        if credential:
            ...
            return (
                jsonify({"success": False, "message": "Invalid credential password"}),
                403,
            )
...
def main():
    ...
    print("Starting Secure Credential Manager Web Server...")
