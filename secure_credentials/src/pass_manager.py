...
try:
    ...
    class Win32SecurityDummy:
        ...
        def ACL(self) -> Any: pass
        def LookupAccountName(self, domain: str, username: str) -> tuple[Any, Any, Any]: pass
        def GetFileSecurity(self, path: str, info_type: int) -> Any: pass
        def SetFileSecurity(self, path: str, info_type: int, security: Any) -> None: pass
...
class Credential:
    def __init__(self, name: str, value: str, cred_type: CredentialType, created_at: datetime.datetime):
        self.name = name
        self.value = value
        self.type = cred_type
...
class CredentialManager:
    def __init__(self, message_callback=None):
        ...
        self.message_callback = message_callback or (lambda x: None)  # Default no-op callback
    ...
    def _ensure_secure_files(self):
        ...
        if WINDOWS_SECURITY_AVAILABLE:
            ...
            self.message_callback("Warning: pywin32 not installed. File permissions not secured.")
            ...
            try:
                ...
                self.message_callback(f"Warning: Could not set file permissions: {str(e)}")
    ...
    def backup_credential_file(self):
        ...
        try:
            with open(self.credential_file_path, 'r') as original, open(backup_path, 'w') as backup:
    ...
    def initialize_store(self, master_password):
        ...
        try:
            ...
            if not os.path.exists(self.credential_file_path) or os.path.getsize(self.credential_file_path) == 0:
                self.message_callback("First time setup - creating new credential store...")
                # Create a test credential to verify the master password works
                test_data = {
                    "_master_verify": {
                        "value": pwl.password_encrypt(b"verification", master_password).decode('utf-8'),
                        "type": "system",
                ...
                if not existing_creds:
                    ...
                    self.message_callback("Credential store is empty, reinitializing...")
                    test_data = {
                        "_master_verify": {
                            "value": pwl.password_encrypt(b"verification", master_password).decode('utf-8'),
                            "type": "system",
                    ...
                    self.write_creds(test_data)
                    self.message_callback("Credential store reinitialized successfully.")
                    return True
                    
                if "_master_verify" not in existing_creds:
                    # If verification token is missing, reinitialize
                    self.message_callback("Verification token missing, reinitializing...")
                    test_data = {
                        "_master_verify": {
                            "value": pwl.password_encrypt(b"verification", master_password).decode('utf-8'),
                            "type": "system",
                    ...
                    self.message_callback("Credential store reinitialized while preserving existing credentials.")
                    return True
    ...
    def enter_and_encrypt_credential(self, name, value, cred_type, password):
        ...
        if not re.match(r'^[a-zA-Z0-9_]+$', name):
            raise ValueError('Invalid format. Use only letters, numbers, and underscores.')
        ...
        encrypted_value = pwl.password_encrypt(message=bytes(value, 'utf-8'), password=password)
        ...
        print(f"Added credential {name} to credential file and {self.wizardy_file_path}")
    ...
    def decrypt_credential(self, credential_ref, pw_decryptor):
        ...
        if credential_ref in existing_creds:
            ...
            try:
                ...
                existing_creds[credential_ref]['last_accessed'] = self.datetime_current_UTC().isoformat()
                self.write_creds(existing_creds)
    ...
    def edit_credential(self, old_name: str, new_name: str, new_value: str, new_type: str, password: str):
        ...
        encrypted_value = pwl.password_encrypt(message=bytes(new_value, 'utf-8'), password=password)
        
        # Update or create the credential entry
        credential_data = {
            'value': encrypted_value.decode('utf-8'),
            'type': new_type,
            'created_at': existing_creds[old_name]['created_at'],  # Preserve creation date
