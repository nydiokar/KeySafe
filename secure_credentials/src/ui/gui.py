...
class CredentialManagerGUI:
    ...
    def check_visibility(self):
        ...
        if self.visibility_needed and hasattr(self, 'root') and self.root.winfo_exists():
    ...
    def show_main_window(self):
        ...
        if hasattr(self, 'root') and self.root.winfo_exists():
            ...
            # Log window state
            self.logger.debug(f"Window state after show_main_window: exists={self.root.winfo_exists()}, "
    ...
    def setup_system_tray(self):
        ...
        try:
            ...
            self.icon = Icon("SecureCredManager", image, "Secure Credential Manager", menu)
            # Start the system tray in a different thread without waiting
            self.icon.run_detached()
        except ImportError:
            print("System tray support not available. Install pystray package for system tray functionality.")
    ...
    def lock(self):
        ...
        # Add lock message
        ttk.Label(self.lock_window, text="Application Locked", font=('Helvetica', 12, 'bold')).pack(pady=10)
        ttk.Label(self.lock_window, text="Enter master password to unlock").pack(pady=5)
        
        # Password entry
        self.lock_password_var = ttk.StringVar()
        password_entry = ttk.Entry(self.lock_window, textvariable=self.lock_password_var, show="*")
        password_entry.pack(fill=X, padx=20, pady=5)
        
        # Unlock button
        ttk.Button(self.lock_window, text="Unlock", 
    ...
    def load_window_state(self):
        ...
        config_path = os.path.join(os.path.expanduser("~"), ".secure_credentials", "gui_config.json")
        try:
            with open(config_path, 'r') as f:
                config = json.load(f)
                self.root.geometry(f"{config.get('width', 1000)}x{config.get('height', 700)}")
    ...
    def create_menu_bar(self):
        ...
        file_menu.add_command(label="New Credential", command=self.add_credential, accelerator="Ctrl+N")
        ...
        # Auto-lock timeout
        self.auto_lock_var = ttk.StringVar(value=str(self.load_setting('auto_lock_timeout', 300) // 60))
        security_menu.add_command(
            label="Set Auto-Lock Timeout...",
            command=self.set_auto_lock_timeout
        )
        
        # Clear clipboard
        self.clear_clipboard_var = ttk.BooleanVar(value=self.load_setting('clear_clipboard', True))
        security_menu.add_checkbutton(
            label="Auto-Clear Clipboard",
            variable=self.clear_clipboard_var,
            command=lambda: self.save_setting('clear_clipboard', self.clear_clipboard_var.get())
        )
        ...
        # Updates settings
        self.auto_update_var = ttk.BooleanVar(value=self.load_setting('auto_update', True))
        settings_menu.add_checkbutton(
            label="Check for Updates Automatically",
            variable=self.auto_update_var,
            command=lambda: self.save_setting('auto_update', self.auto_update_var.get())
        )
        settings_menu.add_command(label="Check for Updates Now", command=self.manual_check_for_updates)
        
        # Backup operations
        settings_menu.add_separator()
        settings_menu.add_command(label="Create Backup...", command=self.create_backup)
        settings_menu.add_command(label="Restore from Backup...", command=self.restore_backup)
        ...
        help_menu.add_command(label="Quick Start Guide", command=lambda: self.show_documentation("quick_start"))
        help_menu.add_command(label="Security Guide", command=lambda: self.show_documentation("security"))
        help_menu.add_command(label="Keyboard Shortcuts", command=lambda: self.show_documentation("shortcuts"))
    ...
    def set_auto_lock_timeout(self):
        ...
        ttk.Button(main_frame, text="Save", command=save_timeout,
                  style='primary.TButton').pack(side=RIGHT, padx=5)
        ttk.Button(main_frame, text="Cancel", command=dialog.destroy,
    ...
    def create_toolbar(self):
        ...
        # Buttons
        ttk.Button(toolbar, text="New", command=self.add_credential, style='primary.TButton').pack(side=RIGHT, padx=5)
        ttk.Button(toolbar, text="View", command=self.view_credential, style='info.TButton').pack(side=RIGHT, padx=5)
        ttk.Button(toolbar, text="Edit", command=self.edit_credential, style='info.TButton').pack(side=RIGHT, padx=5)
        ttk.Button(toolbar, text="Delete", command=self.delete_credential, style='danger.TButton').pack(side=RIGHT, padx=5)
        ttk.Button(toolbar, text="Copy", command=self.copy_to_clipboard, style='success.TButton').pack(side=RIGHT, padx=5)
    ...
    def sort_treeview(self, tree, col, reverse):
        ...
        items = [(tree.set(k, col), k) if col != '#0' else (tree.item(k)['text'], k) 
    ...
    def load_credentials(self):
        ...
        for name, data in creds.items():
            ...
            last_accessed = self.format_date(last_accessed) if last_accessed else 'Never'

            # Add to main treeview
            self.tree.insert('', 'end', text=name, values=(cred_type, created, last_accessed),
                           tags=(cred_type,))

            # Add to type-specific tree
            if cred_type == 'password':
                self.password_tree.insert('', 'end', text=name, values=(cred_type, created, last_accessed),
                                        tags=(cred_type,))
                counts['password'] += 1
            elif cred_type == 'api_key':
                self.apikey_tree.insert('', 'end', text=name, values=(cred_type, created, last_accessed),
                                      tags=(cred_type,))
                counts['api_key'] += 1
            else:
                self.other_tree.insert('', 'end', text=name, values=(cred_type, created, last_accessed),
                                     tags=(cred_type,))
                counts['other'] += 1
    ...
    def get_master_password(self):
        ...
        if is_new_store:
            ttk.Label(main_frame, text="First Time Setup - Create Master Password", 
                     font=('Helvetica', 10, 'bold')).pack(pady=(0, 10))
            ttk.Label(main_frame, text="This password will be used to encrypt your credential storage.\n"
        ...
        ttk.Button(main_frame, text="OK", command=on_ok,
                  style='primary.TButton').pack(side=RIGHT, padx=5)
        ttk.Button(main_frame, text="Cancel", command=on_cancel,
                  style='secondary.TButton').pack(side=RIGHT, padx=5)
    ...
    def add_credential(self):
        ...
        if dialog.result:
            ...
            try:
                if self.keepass.add_credential(name, value, cred_type, credential_password):
                    self.load_credentials()
    ...
    def edit_credential(self):
        ...
        if dialog.result:
            ...
            try:
                if self.keepass.edit_credential(name, new_name, new_value, new_type, new_password):
                    self.load_credentials()
    ...
    def delete_credential(self):
        ...
        if messagebox.askyesno("Confirm", "Are you sure you want to delete this credential?"):
            try:
                if self.keepass.delete_credential(name, password):  # Use credential password instead of master password
                    self.load_credentials()
                    self.update_status(f"Deleted credential: {name}")
                else:
                    self.update_status("Failed to delete credential - incorrect password")
            except Exception as e:
                self.update_status(f"Error: {str(e)}")
    ...
    def copy_to_clipboard(self):
        ...
        credential = self.keepass.get_credential(name, password)  # Use credential password
        if credential:
    ...
    def on_select(self, event):
        ...
        if selected:
            self.update_status("Selected credential: " + event.widget.item(selected[0])['text'])
    ...
    def show_credential_details(self, credential):
        ...
        value_entry = ttk.Entry(value_frame, textvariable=value_var, show="*", state="readonly")
        value_entry.pack(side=LEFT, fill=X, expand=YES)
        
        # Show/Hide value button
        show_var = ttk.BooleanVar(value=False)
        ttk.Checkbutton(value_frame, text="Show", 
                       variable=show_var, 
                       command=lambda: value_entry.configure(show="" if show_var.get() else "*"),
                       style='primary.TCheckbutton').pack(side=LEFT, padx=(5, 0))
        ...
        ttk.Button(main_frame, text="Copy to Clipboard", 
                  command=lambda: [pyperclip.copy(credential['value']), 
    ...
    def manual_check_for_updates(self):
        ...
        try:
            if self.check_for_updates():
                ...
                messagebox.showinfo("Updates", "Updates are available. Please download the latest version.")
        except Exception as e:
    ...
    def show_settings(self):
        ...
        messagebox.showinfo("Settings", "Settings dialog will be implemented in a future version.")
        
    def show_password_generator(self):
        """Show the password generator dialog."""
        messagebox.showinfo("Password Generator", "Password generator will be implemented in a future version.")
        
    def run_security_check(self):
        """Run a security check on stored credentials."""
        messagebox.showinfo("Security Check", "Security check will be implemented in a future version.")
...
"""
    ...
    def show_about(self):
        ...
        messagebox.showinfo("About", "Secure Credential Manager\nVersion 1.0.0\n\nA secure password manager.")
        
    def import_credentials(self):
        """Import credentials from a file."""
        messagebox.showinfo("Import", "Import functionality will be implemented in a future version.")
        
    def export_credentials(self):
        """Export credentials to a file."""
        messagebox.showinfo("Export", "Export functionality will be implemented in a future version.")
        
    def copy_field(self, field_type: str):
        """Copy a field to the clipboard."""
        messagebox.showinfo("Copy", f"Copy {field_type} functionality will be implemented in a future version.")

    def check_for_updates(self) -> bool:
        """Check for updates and return True if up to date, False if updates available."""
        try:
            # Get current version from package
            current_version = pkg_resources.get_distribution('secure_credentials').version
...
class CredentialDialog:
    def __init__(self, parent, name=None, current_value=None):
        ...
        ttk.Checkbutton(password_frame, text="Show", 
                       variable=self.show_password_var, 
                       command=lambda: self.toggle_show(self.password_entry, self.show_password_var),
                       style='primary.TCheckbutton').pack(side=LEFT, padx=(5, 0))
        ...
        type_combo = ttk.Combobox(type_frame, textvariable=self.type_var, 
                                values=[CredentialType.PASSWORD, CredentialType.API_KEY, CredentialType.OTHER],
                                state="readonly")
    ...
    def ok(self):
        if not self.name_var.get() or not self.value_var.get() or not self.password_var.get():
            messagebox.showwarning("Warning", "Please fill in all fields")
            return
            
        self.result = (self.name_var.get(), self.value_var.get(), self.type_var.get(), self.password_var.get())
        self.dialog.destroy()